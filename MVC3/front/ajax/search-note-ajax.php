...
$all_note_query = "SELECT sn.*,snr.ratings FROM sellernotes sn LEFT JOIN sellernotesview snr ON sn.id=snr.noteid WHERE sn.status=3 AND sn.title LIKE '%$selected_search%'";
$query_append = "";

//to append all the query
($selected_type != 0 && $selected_type != "")
    ? $query_append .= " AND sn.notetype='$selected_type'" : "";

($selected_category != 0 && $selected_category != "")
    ? $query_append .= " AND sn.category='$selected_category'" : "";

($selected_university != 0 && $selected_university != "")
    ? $query_append .= " AND sn.university='$selected_university'" : "";

($selected_course != 0 && $selected_course != "")
    ? $query_append .= " AND sn.course='$selected_course'" : "";

($selected_country != 0 && $selected_country != "")
    ? $query_append .= " AND sn.country='$selected_country'" : "";

($selected_rating != 0 && $selected_rating != "")
    ? $query_append .= " AND snr.ratings>$selected_rating " : "";
        ...
        <div class="row">
            ...
            <div class="col-lg-4 col-md-6 col-sm-6 col-12 single-book-selecter">
                ...
                <div class="search-result-below-img">
                    ...
                    <!-- imappropriate count -->
                    <div class="search-result-data">
                        <?php $appropriate_query = mysqli_query($con, "SELECT * FROM sellernotesreport WHERE noteid=$note_id");
                    ...
                    <?php
                        // display rating
                        $ratiing_getter = mysqli_query($con, "SELECT AVG(ratings),COUNT(ratings) FROM sellernotesview WHERE noteid=$note_id");
